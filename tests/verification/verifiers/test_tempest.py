...
class TempestTestCase(test.TestCase):
    ...
    def setUp(self):
        ...
        self.verifier.log_file = '/tmp/testr_log.xml'
        self.set_name = 'smoke'
        self.regex = None
    ...
    @mock.patch(TEMPEST_PATH + '.tempest.Tempest._generate_env')
    ...
    @mock.patch(TEMPEST_PATH + '.tempest.Tempest._check_testr_initialization')
    @mock.patch(TEMPEST_PATH + '.tempest.Tempest._run')
    ...
    def test_verify(self, mock_endpoint, mock_osclients, mock_get,
                    mock_gen, mock_write, mock_run, mock_testr_init):
        fake_conf = mock.MagicMock()
        mock_gen.return_value = fake_conf

        self.verifier.verify(self.set_name, None)

        mock_gen.assert_called_once()
        mock_write.assert_called_once_with(fake_conf)
        mock_run.assert_called_once_with(self.set_name)
    ...
    @mock.patch('os.path.isdir')
    @mock.patch(TEMPEST_PATH + '.tempest.subprocess.call')
