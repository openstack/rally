...
class SFCRemovePortpair(vcpe_utils.vCPEScenario, neutron_utils.NeutronScenario, nova_utils.NovaScenario, scenario.OpenStackScenario):
    ...
    def run(self, src_cidr, dest_cidr, image, flavor, public_net, username, password):
        ...
        kwargs.update({'nics': nics})
        kwargs.update({'key_name': key_name})
        service_vm11 = self._boot_server(image, flavor, False, **kwargs)
        ...
        kwargs.update({'nics': nics})
        kwargs.update({'key_name': key_name})
        service_vm12 = self._boot_server(image, flavor, False, **kwargs)
        ...
        kwargs.update({'nics': nics})
        kwargs.update({'key_name': key_name})
        service_vm13 = self._boot_server(image, flavor, False, **kwargs)
