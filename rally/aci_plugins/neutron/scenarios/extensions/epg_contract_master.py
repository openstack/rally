...
class SVIBGPConnectivity(create_ostack_resources.CreateOstackResources, vcpe_utils.vCPEScenario, neutron_utils.NeutronScenario,
   ...
   def run(self, cidr1, cidr2, image, flavor, public_net, aci_nodes, username, password, apic):
          ...
          try:
                ...
                net, sub = self._create_network_and_subnets({"provider:network_type": "vlan"}, {"cidr": cidr1}, 1, None)
                epg_name = 'net_' +  net['network']['id']
                net1, sub1 = self._create_network_and_subnets({"provider:network_type": "vlan", "apic:epg_contract_masters": [{'app_profile_name': 'OpenStack','name': epg_name}]}, {"cidr": cidr1}, 1, None)
