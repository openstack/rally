...
class L3OutBGPConnectivity(create_ostack_resources.CreateOstackResources, vcpe_utils.vCPEScenario, neutron_utils.NeutronScenario,
    ...
    def run(self, access_network, nat_network, image, flavor, username, password, dualstack):
        ...
        if dualstack:
            ...
            command3 = {
                        "interpreter": "/bin/sh",
                        "script_inline": "chmod +x /root/create_bird.sh;/root/create_bird.sh " + fip1[0] +" "+ fip1[1]
                    }

            command4 = {
                        "interpreter": "/bin/sh",
                        "script_inline": "chmod +x /root/create_bird.sh;/root/create_bird.sh " + fip2[0] +" "+ fip2[1]
                    }
        else:
            command1 = {
                        "interpreter": "/bin/sh",
            ...
            command3 = {
                        "interpreter": "/bin/sh",
                        "script_inline": "chmod +x /root/create_bird.sh;/root/create_bird.sh " + fip1[0]
                    }

            command4 = {
                        "interpreter": "/bin/sh",
                        "script_inline": "chmod +x /root/create_bird.sh;/root/create_bird.sh " + fip2[0]
                    }
        print("Configuring the VMs...")
        self._remote_command(username, password, fip1[0], command1, access_vm)
        self._remote_command(username, password, fip2[0], command2, nat_vm)
        self._remote_command(username, password, fip1[0], command3, access_vm)
        self._remote_command(username, password, fip2[0], command4, nat_vm)
