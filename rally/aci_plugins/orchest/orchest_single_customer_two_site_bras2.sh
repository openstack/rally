...
gen_bird_conf(){
    ...
    cat /etc/bird/bird-eth0.tmpl | sed -e "s/_MY_IP_TMPL/\$myip/" | sed -e "s/10.10.251.0\/24/10.10.251.4\/30/" >/etc/bird/bird-eth0.conf
}
...
gen_customer_bird_conf(){
    ...
    cat /etc/bird/bird-cats.tmpl | sed -e "s/_MY_IP_TMPL/\$myip/" | sed -e "s/2010/\$asn/"| sed -e "s/10.0.1.0/10.0.2.0/" >/etc/bird/bird-\$name.conf
}
