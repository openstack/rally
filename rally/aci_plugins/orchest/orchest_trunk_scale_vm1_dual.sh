...
mkbras() {
    ...
    ip -6 route add 2001:a0::/64 via 2001:a0::1
...
set -x
    ...
    do
	HEXVID=\$(printf "%x\n" \$VID)
        ip netns exec "cust-\$VID" ping -c 3 192.168.\$VID.102
	ip netns exec "cust-\$VID" ping6 -c 3 2001:\$HEXVID::66
        ping -c 3 192.168.\$VID.102
	ping6 -c 3 2001:\$HEXVID::66
        ping -c 3 192.168.\$VID.101
	ping6 -c 3 2001:\$HEXVID::65
	VID=\$(expr "\$VID" + 1)
    done
